...
class IfaceNamesService:
    ...
    def write_udev_rules():
        ...
        # Separate eth and wlan registered adapters
        eth_adapters = {n: i for n, i in sorted(interfaces.items()) if n.startswith("eth") and not i.get("builtin")}
        wlan_adapters = {n: i for n, i in sorted(interfaces.items()) if n.startswith("wlan") and not i.get("builtin")}

        lines = [
            "# RaspSec — Persistent network interface naming",
            "#",
            "# Built-in: eth0 (driver-based), wlan0 (built-in WiFi)",
            "# USB adapters: ethX / wlanX (MAC-based, persistent)",
        ...
        for name, info in {**eth_adapters, **wlan_adapters}.items():
            mac = info.get("mac", "")
            if not mac:
                continue
